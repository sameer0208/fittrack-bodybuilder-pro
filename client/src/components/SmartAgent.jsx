...
import { Sparkles, X, Send, Trash2, Loader2, Bot, UserCircle } from 'lucide-react';
    ...
    <>
      ...
      {!open && (
        <button
          ...
          className="fixed bottom-20 right-4 lg:bottom-6 lg:right-6 z-50 w-14 h-14 rounded-full bg-gradient-to-br from-indigo-500 to-purple-600 text-white shadow-lg shadow-indigo-500/40 flex items-center justify-center hover:scale-105 active:scale-95 transition-transform touch-manipulation"
      ...
      {open && (
        <div className="fixed inset-0 z-[90] flex items-end justify-end lg:items-end lg:justify-end">
          ...
          <div className="relative w-full h-[85vh] max-h-[85vh] lg:w-[420px] lg:h-[620px] lg:max-h-[80vh] lg:m-6 lg:mb-6 bg-slate-800 border border-slate-700 rounded-t-2xl lg:rounded-2xl shadow-2xl flex flex-col overflow-hidden animate-slide-up">
            {/* Header */}
            <div className="flex items-center justify-between px-4 py-3 bg-gradient-to-r from-indigo-600 to-purple-600 shrink-0">
              <div className="flex items-center gap-3">
                <div className="w-9 h-9 bg-white/20 rounded-xl flex items-center justify-center">
                  <Bot size={20} className="text-white" />
                </div>
                <div>
                  <h3 className="font-bold text-white text-sm leading-tight">FitTrack AI Agent</h3>
                  <p className="text-[11px] text-white/70">Fitness & Nutrition Expert</p>
                </div>
              </div>
            ...
            <div ref={scrollRef} className="flex-1 overflow-y-auto px-4 py-4 space-y-4">
              {messages.length === 0 && !loading && (
                <div className="flex flex-col items-center justify-center py-8 text-center">
                  ...
                  <h4 className="text-white font-bold text-base mb-1">Hey! I'm your AI fitness assistant</h4>
                  <p className="text-slate-400 text-sm mb-6 max-w-[260px]">
                    Ask me anything about workouts, nutrition, supplements, or health goals.
                  </p>
            ...
            <div className="px-4 pb-4 pt-2 border-t border-slate-700 shrink-0">
              {!token ? (
                <p className="text-xs text-slate-500 text-center py-2">Sign in with an account to use the AI agent.</p>
              ) : (
